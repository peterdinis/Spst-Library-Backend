...
import { CreateNotificationDto, CreateOrderNotificationDto, ReturnOrderNotificationDto } from '@app/dtos';
import { BadRequestException, InternalServerErrorException, NotFoundException } from '@nestjs/common';

type MockModel<T> = {
  create: jest.Mock<T, [Partial<T>]>;
  find: jest.Mock<{ sort: (arg: object) => Promise<Partial<T>[]> }, [object]>;
  findByIdAndUpdate: jest.Mock<Promise<Partial<T> | null>, [string, Partial<T>, object]>;
};
...
describe('NotificationsService', () => {
  ...
  describe('create', () => {
    it('should throw BadRequestException if userId or message missing', async () => {
      const dto: CreateNotificationDto = { userId: '', message: '', type: 'info' };
    ...
    it('should create and return a notification', async () => {
      ...
      mockModel.create.mockReturnValue({ save: saveMock } as unknown as NotificationDocument);

      const result = await service.create(dto);
      expect(result).toEqual(mockSaved);
      expect(messagesService.sendKafkaMessage).toHaveBeenCalledWith(
        'notification.created',
        expect.objectContaining({ id: mockSaved._id, userId: dto.userId, message: dto.message, type: dto.type }),
      );
    });

    it('should throw InternalServerErrorException if save fails', async () => {
      const dto: CreateNotificationDto = { userId: '123', message: 'test', type: 'info' };
  ...
  describe('createOrderNotification', () => {
    it('should throw BadRequestException if userEmail or message missing', async () => {
      const dto: CreateOrderNotificationDto = { userEmail: '', message: '', type: 'order' };
      await expect(service.createOrderNotification(dto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should create and return order notification', async () => {
      ...
      expect(messagesService.sendKafkaMessage).toHaveBeenCalledWith(
        'notification.created',
        expect.objectContaining({ id: mockSaved._id, userEmail: dto.userEmail, message: dto.message, type: dto.type }),
      );
    });
  });
  ...
  describe('createReturnOrderNotification', () => {
    it('should throw BadRequestException if userEmail or message missing', async () => {
      const dto: ReturnOrderNotificationDto = { userEmail: '', message: '', type: 'return' };
      await expect(service.createReturnOrderNotification(dto)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('findByUser', () => {
    ...
    it('should return notifications', async () => {
      ...
      mockModel.find.mockReturnValue({ sort: sortMock } as unknown as { sort: (arg: object) => Promise<Partial<NotificationDocument>[]> });
    ...
    it('should throw InternalServerErrorException if query fails', async () => {
      mockModel.find.mockImplementation(() => { throw new Error('fail'); });
      await expect(service.findByUser('123')).rejects.toThrow(InternalServerErrorException);
    });
  });
  ...
  describe('markAsRead', () => {
    it('should throw BadRequestException if id invalid', async () => {
      await expect(service.markAsRead('invalid-id')).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException if notification not found', async () => {
      mockModel.findByIdAndUpdate.mockResolvedValue(null);
      await expect(service.markAsRead('507f191e810c19729de860ea')).rejects.toThrow(NotFoundException);
    });

    it('should mark notification as read', async () => {
      const mockNotif: Partial<NotificationDocument> = { _id: '507f191e810c19729de860ea', isRead: true };
    ...
    it('should throw InternalServerErrorException if update fails', async () => {
      ...
      await expect(service.markAsRead('507f191e810c19729de860ea')).rejects.toThrow(InternalServerErrorException);
    });
  });
});
