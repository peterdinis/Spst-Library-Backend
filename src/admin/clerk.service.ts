...
export class ClerkService {
    ...
    async getActiveUsers() {
        try {
            const sessions = await this.clerk.sessions.getSessionList({ status: 'active' });
            ...
            const users = await Promise.all(
                userIds.map(async (id) => {
                    try {
                        ...
                        throw new BadRequestException(`Failed to fetch user with ID ${id}: ${err.message || err}`);
                    }
                }),
            );
            ...
            if (validUsers.length === 0) {
                throw new HttpException('No valid active users found', HttpStatus.NOT_FOUND);
            }
